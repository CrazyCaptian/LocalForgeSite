...
const token = eth.contract(tokenABI).at(_CONTRACT_ADDRESS);  //forge
const token2 = eth.contract(tokenABI2).at(_CONTRACT_ADDRESS2); //auctions
...
function calculateNewMiningDifficulty(current_difficulty,
  ...
  var target_eth_blocks_since_last_difficulty_period = epochs_mined.mul(new Eth.BN(60*14));
...
stats = [
 ...
 ['Total 0xBitcoin spent at Auctions',  token2.totalBurnt,                   "0xBTC",   0.00000001, null     ], /* supply */
  ['',                              null,                                 "",                 1,          null     ], /* */
  ['Total APY',  token2.nextDayTime,                   "",   1, null     ], /* supply */
 ['Total Liquidity',  token2.secondsPerDay,                   "Days",   0.00001157407, null     ], /* supply */
 ['',                              null,                                 "",                 1,          null     ], /* */
...
['Current Mining Reward per 48 minute Solve',         token.getMiningReward,                _CONTRACT_SYMBOL,   0.000000000000000001, null     ], /* mining */
 ...
 ['u3',                eth.blockNumber,                      "",                 1,          null     ], /* mining */
   ['u4',   token.latestDifficultyPeriodStarted,  "",                 1,          null     ], /* mining difficulty */
...
function updateStatsThatHaveDependencies(stats) {
  ...
  el_safe('#MinedSupplyRemaininginEra').innerHTML = "<b>" + supply_remaining_in_era.toFixed(0) + "</b> " + _CONTRACT_SYMBOL + " <span style='font-size:0.8em;'>(" +rewards_blocks_remaining_in_era.toFixed(0) + " blocks)</span>";
...
console.log("First _IDEAL_BLOCK_TIME_SECONDS:", _IDEAL_BLOCK_TIME_SECONDS);
  ...
  /* time until next epoch ('halvening') */
  el_safe('#CurrentRewardEra').innerHTML += " <span style='font-size:0.8em;'>(next era: ~" + secondsToReadableTime(rewards_blocks_remaining_in_era * _IDEAL_BLOCK_TIME_SECONDS / 2.2) + " to "+secondsToReadableTime(rewards_blocks_remaining_in_era * _IDEAL_BLOCK_TIME_SECONDS) +")</div>";

  freadsf = getValueFromStats('Tokens distributed via Mining', stats)

  el_safe('#TokensdistributedviaMining').innerHTML = "<b>" + freadsf.toFixed(0) + " </b> Forge";
  ...
  el_safe('#TokensdistributedviaStaking').innerHTML = "<b>" + freadsf.toFixed(0) + " </b> Forge";
  ...
  auction_tot = getValueFromStats('Total Forge Circulating Supply', stats) + getValueFromStats('Current Auction', stats)*8192-8192
  
  el_safe('#TotalForgeCirculatingSupply').innerHTML = "<b>" + auction_tot.toFixed(0) + " </b> Forge";

  auction_totf = getValueFromStats('Tokens distributed via Auctions', stats) - 8192
  el_safe('#TokensdistributedviaAuctions').innerHTML = "<b>" + auction_totf.toFixed(0) + " </b> Forges";
...
el_safe('#TimeRemaininginAuction').innerHTML = "<b>" + '<progress value="'+tt2+'" max="'+freadsffff*24*3600+'"></progress>'+(timeremain).toFixed(1) + " </b> seconds";
 ...
 hashy = getValueFromStats('Mining Difficulty', stats)*2
  ...
  var new_mining_difficulty = hashy ;/*calculateNewMiningDifficulty(difficulty,
                                                           eth_blocks_since_last_difficulty_period,
...
function createStatsTable(){
  stats.forEach(function(stat){
    ...
    stat_multiplier = stat[3]
		if(stat[0]!='u2' && stat[0] != 'u3' && stat[0] != 'u4' && stat[0] != 'u5'){
